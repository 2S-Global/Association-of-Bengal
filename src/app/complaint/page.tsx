...
import React, { useState } from "react";
import { Send, Loader2, CheckCircle2, AlertCircle, ArrowLeft, LifeBuoy, User, Tag, FileText, Copy, Check } from "lucide-react";
import { useRouter } from "next/navigation";

// Matches your Next.js app directory route: src/app/api/complaints
...
export default function MemberComplaintPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Failed to submit your request. Please try again later.");
      }

      // Capture tracking ID from the backend API route
      if (data.trackingId) {
          ...
          <div className="p-6 sm:p-10">
            {submitted ? (
              <div className="text-center py-12 space-y-6 animate-in fade-in zoom-in-95 duration-300">
                ...
                {trackingId && (
                  <div className="max-w-sm mx-auto bg-[#fff8f5] border border-[#e0bfbf] p-4 rounded-xl flex items-center justify-between gap-3 shadow-sm">
                    <div className="text-left">
                      <p className="text-[11px] font-bold uppercase tracking-wider text-[#584141]">Your Tracking ID</p>
                      <p className="text-sm font-mono font-bold text-[#570013]">{trackingId}</p>
                    </div>
                    <button
                      onClick={handleCopyTrackingId}
                      className="p-2.5 rounded-lg bg-white border border-[#e0bfbf] text-[#570013] hover:bg-[#570013] hover:text-white transition-all shadow-xs flex items-center gap-1 text-xs font-semibold"
