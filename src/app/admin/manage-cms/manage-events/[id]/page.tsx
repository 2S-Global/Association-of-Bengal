...
export default function EditEventWorkspacePage() {
  ...
  const parseSafeDate = (dateVal?: string): Date | null => {
  ...
  const fetchEventDetails = async () => {
    try {
      ...
      const list = Array.isArray(json?.data) ? json.data : Array.isArray(json?.events) ? json.events : Array.isArray(json) ? json : [];
      if (json.success || list.length > 0) {
        const found = list.find((ev: AdminEvent) => ev._id === eventId);
        if (found) {
  ...
  const handleDateSelection = (start: Date | null, end: Date | null) => {
    ...
    if (start && start <= today) {
      setErrorMsg("Start date must be from tomorrow onwards (today and past dates are not selectable).");
      return;
    }
