...
export default function AdminFairSettingsPage() {
  ...
  const validateForm = () => {
    if (!selected?.fairTitle?.trim()) {
      setActiveTab("general");
      setErrorMsg("Validation Error: Primary Fair Title is required.");
      return false;
    }
    if (!selected?.fairDates?.trim()) {
      setActiveTab("general");
      setErrorMsg("Validation Error: Fair Dates Range is required.");
      return false;
    }
    const options = selected?.spaceOptions || [];
    if (options.length === 0) {
      setActiveTab("space");
      setErrorMsg("Validation Error: At least one stall tier option is required.");
      return false;
    }
    for (let i = 0; i < options.length; i++) {
      if (!options[i].label?.trim() || !options[i].value?.trim()) {
        setActiveTab("space");
        setErrorMsg(`Validation Error: Stall Tier #${i + 1} has an empty label or value.`);
        return false;
      }
    }
    return true;
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    setTouched({ fairTitle: true, fairDates: true });
    setErrorMsg("");
  ...
  return (
    ...
    >
      ...
      {viewMode === "cards" ? (
        ...
        <div className="space-y-6">
          <div className="bg-gradient-to-r from-white via-[#fff8f5] to-[#fef2eb] p-5 sm:p-6 rounded-3xl border border-[#e0bfbf]/70 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="space-y-1">
              <div className="text-[10px] font-extrabold text-[#775a19] uppercase tracking-[0.2em] flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-amber-600" /> CMS
                ADMINISTRATION
              </div>
              <h1 className="text-lg sm:text-xl font-bold font-['Playfair_Display'] text-[#570013] flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 text-xs font-semibold text-[#570013] hover:text-[#800020] cursor-pointer bg-white px-3.5 py-2 rounded-xl border border-[#e0bfbf]/80 shadow-2xs hover:shadow-xs transition-all shrink-0 self-start sm:self-center"
            >
                  ...
                  <div className="space-y-8 animate-fadeIn">
                    <div className="border border-[#e0bfbf]/60 p-5 sm:p-6 rounded-2xl bg-[#fffaf8]/60 space-y-5">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div>
                          <label className="block text-[11px] font-bold uppercase tracking-wider text-[#584141] mb-2">
                            PRIMARY FAIR TITLE <span className="text-red-600">*</span>
                          </label>
                          <input
                            type="text"
                            value={selected.fairTitle || ""}
                            onBlur={() => setTouched({ ...touched, fairTitle: true })}
                            onChange={(e) =>
                              setSelected({
                                ...selected,
                            ...
                            className={`w-full border p-3.5 rounded-2xl text-sm font-bold font-['Playfair_Display'] bg-white focus:outline-none focus:ring-2 focus:ring-[#570013]/20 ${
                              touched.fairTitle && !selected.fairTitle?.trim()
                          ...
                          {touched.fairTitle && !selected.fairTitle?.trim() && (
                            <p className="text-[10px] text-red-600 mt-1 font-semibold">
                              Primary Fair Title is required.
                            </p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 items-start">
                        <div className="relative">
                          <label className="block text-[11px] font-bold uppercase tracking-wider text-[#584141] mb-2 flex items-center gap-1.5">
                            ...
                            FAIR DATES RANGE (DD/MM/YYYY) <span className="text-red-600">*</span>
                          </label>
                          <div className="grid grid-cols-2 gap-2 relative">
                            <DatePicker
                              ...
                              onChange={(date: Date | null) => {
                                ...
                                setTouched({ ...touched, fairDates: true });
                              }}
                              ...
                              className={`w-full border p-3 rounded-xl text-xs font-semibold bg-white ${
                                touched.fairDates && !selected.fairDates?.trim()
                                  ? "border-red-500 bg-red-50/20"
                                  : "border-[#e0bfbf]"
                              }`}
                            />
                    ...
                    <div className="bg-[#fffaf8] p-5 rounded-2xl border border-[#e0bfbf]/70 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div>
                        <h4 className="text-xs font-bold text-[#570013] uppercase tracking-wide">
                          Dynamic Stall Tier Catalog <span className="text-red-600">*</span>
                        </h4>
                ...
                {activeTab === "general" && (
                  <div className="w-full flex justify-end">
                    <button
                      ...
                      onClick={() => {
                        setTouched({ fairTitle: true, fairDates: true });
                        if (!selected?.fairTitle?.trim() || !selected?.fairDates?.trim()) {
                          setErrorMsg("Please fill in the required fields (Primary Fair Title and Fair Dates Range) before proceeding.");
                          return;
                        }
                        setErrorMsg("");
                        setActiveTab("space");
                      }}
                      className="w-full sm:w-auto bg-[#fbf2ed] hover:bg-[#f3ece9] text-[#570013] border border-[#e0bfbf] px-6 py-4 rounded-2xl font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer transition-all active:scale-95 shadow-2xs"
                    >
                      Next Step: Space Options <ArrowRight className="w-4 h-4 text-[#775a19]" />
                    </button>
                  </div>
                )}
                ...
                {activeTab === "space" && (
                  <div className="w-full flex items-center justify-between gap-3">
                    <button
                      ...
                      className="w-full sm:w-auto bg-white hover:bg-[#fbf2ed] text-[#584141] border border-[#e0bfbf] px-6 py-4 rounded-2xl font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer transition-all active:scale-95 shadow-2xs"
                    >
                      <ArrowLeft className="w-4 h-4 text-[#775a19]" /> Back
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                        const options = selected?.spaceOptions || [];
                        if (options.length === 0 || options.some((o: any) => !o.label?.trim() || !o.value?.trim())) {
                          setErrorMsg("Please ensure all stall tiers have valid labels and values before proceeding.");
                          return;
                        }
                        setErrorMsg("");
                        setActiveTab("terms");
                      ...
                      className="w-full sm:w-auto bg-[#fbf2ed] hover:bg-[#f3ece9] text-[#570013] border border-[#e0bfbf] px-6 py-4 rounded-2xl font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer transition-all active:scale-95 shadow-2xs"
                    >
                      Next Step: Terms &amp; Conditions <ArrowRight className="w-4 h-4 text-[#775a19]" />
                    </button>
                  </div>
                )}
