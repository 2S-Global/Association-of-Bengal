    ...
    <div className="space-y-5">
      ...
      <section className="admin-card flex flex-wrap items-start justify-between gap-4 p-5 sm:p-6">
        <div className="flex items-center gap-4">
          {item.member.photoUrl ? (
            <img src={item.member.photoUrl} alt={item.member.fullName} className="h-[4.5rem] w-[4.5rem] rounded-xl object-cover ring-4 ring-[#570013]/10 dark:ring-[#e8b4b4]/20" />
          ) : (
          ...
          <div>
            <span className="mb-2 inline-flex items-center gap-1.5 rounded-full bg-[#570013]/10 px-2.5 py-1 text-xs font-semibold text-[#570013] dark:bg-[#570013]/20 dark:text-[#e8b4b4]"><UserRound className="h-3.5 w-3.5" />Candidate profile</span>
      ...
      <section className="admin-card overflow-hidden">
        <div className="grid gap-4 border-b border-gray-100 p-5 sm:grid-cols-2 lg:grid-cols-4 dark:border-gray-800 sm:p-6">
          <Info icon={<Building2 className="h-4 w-4" />} label="Election" value={item.election.name} />
          <Info icon={<BadgeCheck className="h-4 w-4" />} label="Applied for" value={item.nomination.position} />
          <Info icon={<UserRound className="h-4 w-4" />} label="Wing" value={item.nomination.wing} />
          <div className="admin-muted-surface rounded-lg border border-gray-200 p-4 dark:border-gray-800"><div className="flex items-center gap-2"><span className="admin-icon-tile h-8 w-8 bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400"><BadgeCheck className="h-4 w-4" /></span><p className="admin-detail-label">Nomination status</p></div><span className={`mt-3 inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${statusStyle}`}>{item.nomination.status.charAt(0).toUpperCase() + item.nomination.status.slice(1)}</span></div>
        </div>

        <div className="grid gap-5 p-5 lg:grid-cols-2 sm:p-6">
          <DetailsCard title="Member profile" icon={<UserRound className="h-4 w-4" />}>
          ...
          <DetailsCard title="Contact & contributions" icon={<Mail className="h-4 w-4" />}>
        ...
        <div className="border-t border-gray-100 p-5 dark:border-gray-800 sm:p-6">
          <div className="flex items-center gap-2"><span className="admin-icon-tile h-8 w-8 bg-[#7b5800]/10 text-[#7b5800] dark:bg-[#7b5800]/20 dark:text-[#ead58a]"><CalendarDays className="h-4 w-4" /></span><p className="text-sm font-semibold text-gray-800 dark:text-white/90">Nomination statement</p></div>
...
function Info({ label, value, icon }: { label: string; value?: string; icon?: ReactNode }) {
  return <div className="admin-muted-surface admin-interactive-card rounded-lg border border-gray-200 p-4 dark:border-gray-800"><div className="flex items-center gap-2"><span className="admin-icon-tile h-8 w-8 bg-[#570013]/10 text-[#570013] dark:bg-[#570013]/20 dark:text-[#e8b4b4]">{icon}</span><p className="admin-detail-label">{label}</p></div><p className="mt-3 text-sm font-semibold text-gray-800 dark:text-gray-200">{value || "-"}</p></div>;
...
function DetailsCard({ title, children, icon }: { title: string; children: ReactNode; icon?: ReactNode }) {
  return <section className="admin-muted-surface rounded-xl border border-gray-200 p-5 dark:border-gray-800"><div className="mb-5 flex items-center gap-2"><span className="admin-icon-tile h-8 w-8 bg-[#570013]/10 text-[#570013] dark:bg-[#570013]/20 dark:text-[#e8b4b4]">{icon}</span><h2 className="admin-section-title">{title}</h2></div><div className="grid gap-5 sm:grid-cols-2">{children}</div></section>;
