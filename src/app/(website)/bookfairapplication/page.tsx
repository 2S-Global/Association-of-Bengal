...
import BookFairApplicationForm from "@/components/stallbookingForm/page";
...
export default async function Page() {
  ...
  const rawConfig =
    ...
    (await FairConfig.findOne({ status: "PUBLISHED" }).lean());

  const fairConfig =
    rawConfig && rawConfig.isActive
      ? {
          fairTitle: rawConfig.fairTitle,
          fairSubtitle: rawConfig.fairSubtitle,
          fairDates: rawConfig.fairDates,
          stallSelectionText: rawConfig.stallSelectionText,
          stallSelectionVenue: rawConfig.stallSelectionVenue,
          organizerName: rawConfig.organizerName,
          organizerSubtext: rawConfig.organizerSubtext,
          logoSrc: rawConfig.logoSrc,
          spaceOptions: rawConfig.spaceOptions || [],
          termsAndConditionsHTML: rawConfig.termsAndConditionsHTML,
          fairTimingsText: rawConfig.fairTimingsText,
        }
      : undefined;

  if (!fairConfig) {
    return (
      <main className="min-h-screen bg-[#fff8f5] flex items-center justify-center p-6 font-['Libre_Franklin']">
        <div className="max-w-lg w-full bg-white rounded-3xl border border-[#e0bfbf] shadow-xl p-8 sm:p-12 text-center relative overflow-hidden">
          <div className="w-16 h-16 bg-[#fbf2ed] border border-[#e0bfbf] rounded-2xl flex items-center justify-center mx-auto mb-6 text-[#570013] shadow-inner">
            <CalendarOff className="w-8 h-8 text-[#775a19]" />
          </div>
          <span className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-[#775a19] bg-[#fed488]/30 px-3 py-1 rounded-full border border-[#fed488]/60">
            Advisory Notice
          </span>
          <h1 className="text-2xl sm:text-3xl font-bold font-['Playfair_Display'] text-[#570013] mt-4 mb-3">
            No Active Fair at this Moment
          </h1>
          <p className="text-xs sm:text-sm text-[#584141] leading-relaxed mb-8">
            Applications for regional stall allocations are currently closed. The administration has not published an active event window in the registry. Please check back prior to the upcoming literary season or reach out via official correspondence.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link
              href="/"
              className="w-full sm:w-auto bg-[#570013] text-white px-7 py-3.5 rounded-xl font-bold text-xs uppercase tracking-wider hover:bg-[#800020] transition-all shadow-md inline-flex items-center justify-center gap-2"
            >
              Return to Cultural Archive <ArrowRight className="w-3.5 h-3.5" />
            </Link>
          </div>
        </div>
      </main>
  ...
  return <BookFairApplicationForm fairConfig={fairConfig} />;
}