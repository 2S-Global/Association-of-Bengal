...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Look for token in standard response paths
      const token = data.data?.accessToken || data.accessToken || data.token;
      const user = data.data?.user || data.user;
      const member = data.data?.member || data.member;

      if (token) {
        // Save token under BOTH common names so the dashboard layout reads it successfully
        localStorage.setItem("token", token);
        localStorage.setItem("accessToken", token);
        ...
        // Immediately push user to the dashboard upon successful login
