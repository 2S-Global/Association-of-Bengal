...
export async function POST(request: Request) {
  try {
    ...
    const currentFairSlug = data.fair_slug || "default-fair";
    data.fair_slug = currentFairSlug;
    data.fair_title = data.fair_title || "International Kolkata Book Fair 2026";

    // --- DUPLICATE SUBMISSION CHECK PER FAIR ONLY ---
    const existingApplication = await Application.findOne({
      fair_slug: currentFairSlug,
      $or: [
        { participant_email: data.participant_email },
        { participant_mobile: data.participant_mobile },
      ],
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "An application for this specific fair using this email or mobile number has already been submitted.",
        },
        { status: 400 }
      );
    }
    ...
    const uploadToCloudinary = async (
      ...
      if (!ALLOWED_MIME_TYPES.includes(file.type)) {
        throw new Error(
          `Invalid file type for ${folder}. Only PDF, JPG, and PNG are allowed.`
        );
      }
    ...
    data.pan_card_doc = await uploadToCloudinary(
      formData.get("pan_card_doc"),
      "pan_cards"
    );
    data.address_proof_doc = await uploadToCloudinary(
      formData.get("address_proof_doc"),
      "address_proofs"
    );
    ...
    try {
      await sendConfirmationEmail(
        ...
        data.declaration_date,
        data.declaration_place,
        data.fair_title // 👈 Dynamically passes the stored fair title ("New Book Fair Title")
      );
    } catch (emailError) {
      console.error("Non-blocking email transmission error:", emailError);
    }
