...
function parseSmsResponse(payload: unknown): {
  ...
  const errorDescription =
    typeof response.ErrorDescription === "string" ? response.ErrorDescription : undefined;
...
async function sendSmsOtp(mobile: string, otp: string): Promise<{ message: string; requestId?: string }> {
  ...
  const senderId = process.env.SMS_CLIENT_ID || "ABLC";
  ...
  try {
    ...
    const memberRecord = requestedMemberId
      ? mongoose.Types.ObjectId.isValid(requestedMemberId)
        ? await Member.findById(requestedMemberId).select("_id memberId user").lean()
        : await Member.findOne({ memberId: requestedMemberId }).select("_id memberId user").lean()
      : userRecord
    ...
    if (memberRecord?._id || requestedMemberId) payload.memberId = memberRecord?._id || requestedMemberId;
    if (userRecord?._id || requestedUserId) payload.userId = userRecord?._id || requestedUserId;
