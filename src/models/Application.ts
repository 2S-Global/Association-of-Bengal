...
export interface IApplication extends Document {
  ...
  amount?: string;
  remark?: string;
  history: IHistoryItem[]; // 👉 Stores all actions step-by-step
}
...
const HistorySchema = new Schema<IHistoryItem>(
  {
    ...
    amount: { type: String, trim: true },
    remark: { type: String, trim: true },
    date: { type: Date, default: Date.now }, // 👉 Stores exact date and time
  }
);
...
const ApplicationSchema: Schema<IApplication> = new Schema(
  {
    ...
    amount: { type: String, trim: true },
    remark: { type: String, trim: true },
    history: [HistorySchema], // 👉 Array recording every single step
  },
  { timestamps: true }
);
