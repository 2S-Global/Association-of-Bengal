...
//   const fallbackFair: FairEventConfig = {
//     slug: "international-kolkata-book-fair-2026",
...
//     ],
//     termsAndConditionsHTML: DEFAULT_TERMS_HTML,
//     fairTimingsText: DEFAULT_FAIR_TIMINGS,
//   };

//   const [activeFair, setActiveFair] = useState<FairEventConfig>(
//     fairConfig || fallbackFair
//   );
...
//           fairConfig.termsAndConditionsHTML?.trim() || DEFAULT_TERMS_HTML,
//         fairTimingsText:
//           fairConfig.fairTimingsText?.trim() || DEFAULT_FAIR_TIMINGS,
//       });
//     } else {
//       fetch("/api/fair-config")
//         .then((r) => r.json())
//         .then((res) => {
...
//               ...res.data,
//               termsAndConditionsHTML:
//                 res.data.termsAndConditionsHTML?.trim() || DEFAULT_TERMS_HTML,
//               fairTimingsText:
//                 res.data.fairTimingsText?.trim() || DEFAULT_FAIR_TIMINGS,
//             });
//           }
//         })
//         .catch(() => {});
...
//   const formSections: SectionConfig[] = useMemo(
//     () => [
//       {
...
//               const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/, valTrimmed = val.trim();
//               return !emailRegex.test(valTrimmed)
...
//             validation: (file) => {
//               if (!file)
//                 return "Please upload the owner address proof document.";
//               if (file.size > 100 * 1024)
//                 return "File size must be less than 100 KB.";
//               return "";
//             },
...
//       new Set(
//         applications.map(
//           (app) => app.fair_title || app.fair_slug || "International Kolkata Book Fair"
//         )
//       )
//     );
//     return ["ALL", ...titles];
...
//       const fairVal =
//         app.fair_title || app.fair_slug || "International Kolkata Book Fair";
//       const matchFair =
//         selectedFairFilter === "ALL" || fairVal === selectedFairFilter;
//       const matchSearch =
...
//       return matchFair && matchSearch;
//     });
//   }, [applications, selectedFairFilter, searchQuery]);
...
//       const dynamicSlug =
//         activeFair.slug ||
//         activeFair.fairTitle?.toLowerCase().replace(/[^a-z0-9]+/g, "-") ||
//         "dynamic-fair";
//       const dynamicTitle =
//         activeFair.fairTitle || "International Kolkata Book Fair";

//       dataToSend.append("fair_slug", dynamicSlug);
//       dataToSend.append("fair_title", dynamicTitle);
//       dataToSend.append(
//         "fairSubtitle",
//         activeFair.fairSubtitle?.trim() || "FORM FOR PARTICIPATION WITH THE BUILT UP STALLS"
//       );
//       dataToSend.append(
//         "logoSrc",
//         activeFair.logoSrc?.trim() || "/images/logo/balc_logo.png"
//       );
...
//                   Click a fair name to filter participant stall bookings.
//                 </p>
//               </div>
...
//             {/* Fair Name Filter Pills */}
//             <div className="flex flex-wrap items-center gap-2 bg-white p-3 rounded-2xl border border-[#e0bfbf] shadow-2xs">
//               <span className="text-[10px] font-extrabold uppercase tracking-wider text-[#570013] px-2 flex items-center gap-1">
//                 <Calendar className="w-3.5 h-3.5 text-[#775a19]" /> Filter by Fair:
//               </span>
//               {availableFairs.map((fairTitle) => (
//                 <button
//                   key={fairTitle}
//                   type="button"
//                   onClick={() => setSelectedFairFilter(fairTitle)}
//                   className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
//                     selectedFairFilter === fairTitle
//                       ? "bg-[#570013] text-white shadow-xs"
//                       : "bg-[#fbf2ed] text-[#584141] hover:bg-[#e0bfbf]/50"
//                   }`}
//                 >
//                   {fairTitle === "ALL" ? "All Fairs" : fairTitle}
//                 </button>
//               ))}
//             </div>
...
//                   No applications match your selected fair or search query.
//                 </p>
//               </div>
//             ) : (
...
//                         <th className="py-4 px-6">Fair / Event</th>
...
//                         >
//                           <td className="py-4 px-6 text-xs font-semibold text-[#775a19]">
//                             {app.fair_title || app.fair_slug || "International Kolkata Book Fair"}
//                           </td>
...
//                 __html: activeFair.termsAndConditionsHTML || DEFAULT_TERMS_HTML,
//               }}
//             />
//           </div>
...
export default function BookFairApplicationForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      dataToSend.append(
        "fairSubtitle",
        activeFair.fairSubtitle?.trim() ||
          "FORM FOR PARTICIPATION WITH THE BUILT UP STALLS",
      );
      dataToSend.append(
        "logoSrc",
        activeFair.logoSrc?.trim() || "/images/logo/balc_logo.png",
      ...
      <main className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 py-6 lg:py-12 bg-[#fff8f5] text-[#1e1b18] font-['Libre_Franklin'] antialiased selection:bg-[#ffdada] selection:text-[#570013]">
        {isAdminView ? (
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-2 bg-white p-3 rounded-2xl border border-[#e0bfbf] shadow-2xs">
              <span className="text-[10px] font-extrabold uppercase tracking-wider text-[#570013] px-2 flex items-center gap-1">
                <Calendar className="w-3.5 h-3.5 text-[#775a19]" /> Filter by
                Fair:
              </span>
              {availableFairs.map((fairTitle) => (
                <button
                ...
                >
                  {fairTitle === "ALL" ? "All Fairs" : toTitleCase(fairTitle)}
                </button>
              ))}
            </div>
          ...
          <>
            ...
            <div className="bg-gradient-to-b from-[#fff8f5] to-[#fbf2ed] rounded-t-3xl p-6 sm:p-8 shadow-sm border-2 border-[#e0bfbf] mb-0 relative z-10 overflow-hidden">
              ...
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 sm:gap-6 mb-6 pt-1">
                <div className="text-left space-y-1.5">
                  <div className="text-base sm:text-lg lg:text-xl font-bold font-['Playfair_Display'] tracking-wide text-[#570013]">
                    {toTitleCase(activeFair.fairTitle)}
                  </div>
                  <div className="inline-block bg-white text-[#570013] px-3 py-1 rounded-full text-[11px] font-bold tracking-wide uppercase border border-[#e0bfbf] shadow-2xs">
                    {activeFair.fairDates}
                  </div>
        ...
        <div className="bg-gradient-to-b from-white via-white to-emerald-50/30 rounded-3xl w-full max-w-md p-6 sm:p-8 flex flex-col items-center text-center shadow-2xl border-2 border-emerald-500/30 relative m-auto space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-sm text-emerald-900/90 leading-relaxed">
              ...
              . Your application form for stall participation in the{" "}
              <strong>{toTitleCase(activeFair.fairTitle)}</strong> has been
              securely registered.
            </p>
          </div>
