...
export default function HeroSectionForm({ heroSection, onCancel, onSaved }: { heroSection: HeroSection; onCancel: () => void; onSaved: (heroSection: HeroSection) => void }) {
  ...
  return <form onSubmit={save} className="mt-6 overflow-hidden rounded-3xl border border-[#e0bfbf]/70 bg-white shadow-sm">
    <div className="flex flex-col gap-4 border-b border-[#eadbd5] bg-gradient-to-r from-[#fffdfc] to-[#fff7f3] px-6 py-5 sm:flex-row sm:items-start sm:justify-between sm:px-8"><div><h2 className="font-['Playfair_Display'] text-xl font-bold text-[#570013]">Edit Hero Section</h2><p className="mt-1 text-xs text-[#6d5550]">Update the text and statistics displayed on the public website.</p></div><button type="button" onClick={onCancel} disabled={saving} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-[#d8c3ba] px-5 py-2.5 text-sm font-bold text-[#684b45] transition hover:bg-[#fff4ef] disabled:opacity-50"><ArrowLeft className="h-4 w-4" /> Back to overview</button></div>
    ...
    <div className="flex justify-end border-t border-[#eadbd5] bg-[#fffdfc] px-6 py-5 sm:px-8"><button disabled={saving} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#570013] px-6 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-[#40000e] disabled:cursor-not-allowed disabled:opacity-50">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Save Changes</button></div>
  </form>;
}
